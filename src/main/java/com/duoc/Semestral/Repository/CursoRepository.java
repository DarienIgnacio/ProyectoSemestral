...
public class CursoRepository {
    ...
    public CursoRepository() {
        cursos.add(new Curso(1,"testCurso1",alumnos,materias));
        cursos.add(new Curso(2,"testCurso2",alumnos,materias));
        cursos.add(new Curso(3,"testCurso3",alumnos,materias));
    }
    ...
    public String updateCurso(Curso curso) {
        for (Curso c : cursos) {
            if (c.getId() == curso.getId()) {
                ...
                c.setListaMateria(curso.getListaMateria());
    ...
    public String getMaterias(int idCurso) {
        for (Curso c : cursos) {
            if (c.getId() == idCurso) {
                return c.getListaMateria().toString();
            }
        }
        return "Curso no encontrado";
    }
